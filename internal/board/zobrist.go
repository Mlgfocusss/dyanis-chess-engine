// Zobrist hashing, compatible with the Polyglot opening-book format
// (http://hgm.nubati.net/book_format.html) and with the python-chess
// reference implementation, which this was cross-checked against: the
...
// numbers defined by the Polyglot book format. Layout:
//
//	[0..767]   piece-on-square: index = 64*pieceIndex + square
//	           pieceIndex = (pieceType-1)*2 + pivot, pivot = 1 for
//	           White, 0 for Black, pieceType = Pawn..King (1..6)
...
//	[772..779] en passant file (a..h), only hashed in when a pawn is
//	           actually positioned to make the capture
//	[780]      side to move: XORed in when it's White's turn
//
// These specific constants aren't arbitrary — every Polyglot-format
// .bin book in the wild was built using this exact table, so a
// engine has to reproduce it exactly (not "some" random 64-bit
// numbers) to compute hashes that will actually match entries in
// downloaded books like gm2001.bin or komodo.bin.
...
// pieceIndex maps a piece to Polyglot's piece-kind numbering: 0..11,
// ordered black pawn, white pawn, black knight, white knight, ...,
...
func (b *Board) Hash() uint64 {
	...
	for sq, p := range b.Squares {
		...
		h ^= polyglotRandom64[64*pieceIndex(p)+sq]
	}

	if b.Castling&WhiteKingside != 0 {
		h ^= polyglotRandom64[768]
	}
	if b.Castling&WhiteQueenside != 0 {
		h ^= polyglotRandom64[769]
	}
	if b.Castling&BlackKingside != 0 {
		h ^= polyglotRandom64[770]
	}
	if b.Castling&BlackQueenside != 0 {
		h ^= polyglotRandom64[771]
	}

	if b.EnPassant != NoSquare && enPassantCaptureIsPossible(b) {
		h ^= polyglotRandom64[772+b.EnPassant.File()]
	}

	if b.SideToMove == White {
		h ^= polyglotRandom64[780]
	}
...
// side to move is actually positioned to capture on b.EnPassant.
// Polyglot only hashes in the en passant file when this holds — not
// merely whenever EnPassant != NoSquare — so two positions that only
// differ by "a two-square pawn push happened but no enemy pawn was
// adjacent to exploit it" hash identically, matching what every other
// Polyglot-compatible tool does. Whether the capture would leave the
// king in check is irrelevant here; only geometric possibility counts.
func enPassantCaptureIsPossible(b *Board) bool {
	capturingRank := b.EnPassant.Rank() - 1
	if b.SideToMove == Black {
		capturingRank = b.EnPassant.Rank() + 1
	}

	pawn := MakePiece(Pawn, b.SideToMove)
